...
# Include paths
INCLUDE_PATHS	+= $(SDK_INCLUDE_PATH) $(SDK_INCLUDE_PATH)/gcc
INCLUDE_PATHS += $(SDK_INCLUDE_PATH)/app_common $(SDK_INCLUDE_PATH)/s110 $(SDK_INCLUDE_PATH)/sd_common
INCLUDE_PATHS += $(SDK_INCLUDE_PATH)/ble $(SDK_INCLUDE_PATH)/ble/ble_services 
INCLUDE_PATHS += $(PROJECT_INCLUDE_PATHS)
INCLUDES		= $(addprefix -I, $(INCLUDE_PATHS))

# Compiler flags
# CFLAGS = -mcpu=$(CPU) -mthumb -mfloat-abi=soft -Wall -Werror -c
CFLAGS = -mcpu=$(CPU) -mthumb -mfloat-abi=soft -Wall -c
# CFLAGS += -std=c99
...
C_SOURCE_PATHS		+= $(SDK_SOURCE_PATH)/app_common $(SDK_SOURCE_PATH)/ble $(SDK_SOURCE_PATH)/ble/ble_services
